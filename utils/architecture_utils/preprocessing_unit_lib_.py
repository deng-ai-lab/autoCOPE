...
class unit_lib(object):
    def __init__(self, args):
        ...
        self.robj = robj
        self.seurat = importr('future')
        self.seurat = importr('Seurat')
        self.CreateSeuratObject = robj.r('function(x) CreateSeuratObject(counts = x)')
        self.seurat_norm = robj.r('function(x) NormalizeData(x)')
        self.set_variable_features = robj.r('''function(x) {
                                                 VariableFeatures(object = x) <- rownames(x = x)
                                                 return(x)}''')
        self.sparse = robj.r('function(matrix) as(matrix, "sparseMatrix")')
        ...
        #self.integrate_seurat = robj.r('''function(seurat_object_list) {suppressPackageStartupMessages(require(R.utils))
        #                                                                res <- NULL
        #                                                                tryCatch({withTimeout({anchors <- FindIntegrationAnchors(object.list = seurat_object_list)
        #                                                                                       res <- IntegrateData(anchorset = anchors)
        #                                                                                      }
        #                                                                                      , timeout = 100
        #                                                                                     )
        #                                                                         }
        ...
        #                                ''')
        #self.integrate_seurat = robj.r('''function(seurat_object_list) {suppressPackageStartupMessages(require(R.utils))
        #                                                                res <- withTimeout({anchors <- FindIntegrationAnchors(object.list = seurat_object_list)
        ...
        #                                                                }
        #                                ''')
        self.integrate_seurat = robj.r('''function(seurat_object_list) {library(future)
                                       ...
                                       ''')
        self.list_extraction = robj.r('function(integrated) as.matrix(integrated@assays$integrated@data)')
